...
- **Explicitly deferred past the iOS build (decision, 2026-09-17)**: this
  was flagged as possibly more urgent than starting iOS work at all —
  decided instead to proceed with iOS first and pick up recovery-code
  redemption afterward, as tracked future work (see §13 M7a and §15).
  Rationale: today's only two users each already have a working device
  (Android in daily use, iOS about to exist), so the "lost my only device"
  scenario this closes isn't imminent; getting the second person onto an
  iPhone is the nearer-term goal.
...
2. **Device pairing** (`auth.createPairingSession`/`submitPairingDevice`/`getPairingStatus`/`isPairingComplete`, `app/src/components/DevicePairingModal.js`, `UnlockScreen`'s "Pair with another device") — the **primary** way to add a device going forward, no pre-saved secret required at all. An already-unlocked device generates an **8-character human-typed code** (Crockford base32 — 0-9, A-Z minus I/L/O/U — decision, 2026-09-23, revised down from an original ~80-character concatenated-UUID+256-bit-key design that was unusable by hand): 4 characters are a lookup code the server sees (finds the pairing session, meaningless on its own), 4 are a secret the server never sees, generated client-side and relayed only through the human typing/reading the full code aloud. The new device reads/types that code, generates its own keypair, and computes an HMAC (BLAKE2b-stretched to crypto_auth's required 32-byte key) of its public key keyed by the 4-character secret. The granting device polls, recomputes that HMAC locally, and only wraps the real DEK if it matches — this is what stops a compromised server from substituting its own public key mid-relay to hijack the pairing. Once verified, the granting device calls the **same `grantAccess`** used for invite redemption — no separate "complete pairing" action needed. **Accepted trade-off**: 4 characters of this alphabet is ~20 bits of secret entropy, far below the original design's 256 bits — acceptable because the protocol only accepts one submission per session (a second submission to the same code is rejected), so an attacker gets a single blind guess within the 10-minute session window, not an offline search.
...
- ~~iPhone distribution: free Apple ID local signing vs. paid Apple Developer account?~~ **(2026-09-17)** Starting with a **free Apple ID + local Xcode signing** for the first iOS build — fastest path to something running on the household's iPhone, at the cost of a 7-day re-signing cadence and needing the Mac present to renew. Moving to a **paid Apple Developer account ($99/yr) + EAS cloud builds** (TestFlight, build-from-anywhere, no re-signing) is the planned next step once the free path proves the app actually works on iOS, not a permanent choice.
